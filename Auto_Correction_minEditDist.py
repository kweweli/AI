...
def get_probs(word_count_dict):
    ...
    '''
    probs = {} # dict to hold probabilities
...
def switch_letter(word, verbose=False):
    ...
    for L, R in split_l:
        if len(R) not in (0, length):
            fchar = R[0:1]
            nchar = L[-1:]
            beg   = L[:-1]
            end   = R[1:]
            switch_l.append(beg + fchar + nchar + end)
...
def replace_letter(word, verbose=False):
    ...
    for i in range(len(word)):
        for rep in letters:
            replace_l.append(word[:i] + rep + word[i+1:])
...
def insert_letter(word, verbose=False):
    ...
    for i in range(length+1):
        for rep in letters:
            insert_l.append(word[:i] + rep + word[i:])   
...
def edit_one_letter(word, allow_switches = True):
    ...
    ### START CODE HERE ###
    edit_one_set = insert_letter(word, False) + delete_letter(word) + replace_letter(word)
...
def edit_two_letters(word, allow_switches = True):
    ...
    for i in insert_letter(word, False):
        edit_two_l.append(i)
        for j in insert_letter(i, False):
            edit_two_l.append(j)
            
    for i in delete_letter(word):
        edit_two_l.append(i)
        for j in delete_letter(i):
            edit_two_l.append(j)
            
    for i in replace_letter(word):
        edit_two_l.append(i)
        for j in replace_letter(i):
            edit_two_l.append(j)
    
    if allow_switches:
        for i in switch_letter(word):
            edit_two_l.append(i)
            for j in switch_letter(i):
                edit_two_l.append(j)
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    ### START CODE HERE ###
    edit_one_intersect = set(edit_one_letter(word)).intersection(vocab)
    edit_two_intersect = set(edit_two_letters(word)).intersection(vocab)
    
    if word in vocab:
        suggestions.append((word, probs.get(i)))
        
    elif edit_one_intersect is not None:
        for i in edit_one_intersect:
            suggestions.append((i, probs.get(i)))
                
    elif edit_two_intersect is not None:
        for i in edit_two_intersect:
            suggestions.append((i, probs.get(i)))
            
    else: suggestions.append((word, 0))
        
    n_best = sorted(suggestions, key = lambda x: -x[1])[:n] 
...
def min_edit_distance(source, target, ins_cost = 1, del_cost = 1, rep_cost = 2):
    ...
    for row in range(1, m+1): 
        ...
        for col in range(1, n+1):
            ...
            D[row,col] = np.min([D[row-1, col] + del_cost, D[row, col-1] + ins_cost,
