        ...
        if word in vocab:
            ...
            ### START CODE HERE (Replace instances of 'None' with your code) ###
                ...
                # get the emission count of the (pos,word) tuple 
                    count = emission_counts[key] # None

                    # keep track of the POS with the largest count
                    if count > count_final: # None # complete this line

                        # update the final count (largest count)
                        count_final = count # None

                        # update the final POS
                        pos_final = pos # None

            # If the final POS (with the largest count) matches the true POS:
            if (pos_final, word) == (true_label, word): # None # complete this line
...
def create_transition_matrix(alpha, tag_counts, transition_counts):
    ...
    for i in range(num_tags):
        ...
        for j in range(num_tags):
            ...
            key = (all_tags[i], all_tags[j]) #None
    ...
    for i in range(1, len(test_corpus)): 
        ...
        for j in range(num_tags): # complete this line
            ...
            for k in range(num_tags): # complete this line
                ...
                if prob > best_prob_i: # None # complete this line
                    
                    # Keep track of the best probability
                    best_prob_i = prob #None
            ...
            best_probs[j,i] = np.max(best_prob_i) #None
...
def viterbi_backward(best_probs, best_paths, corpus, states):
    ...
    for k in range(num_tags): # complete this line
        ...
        if np.max(best_probs[k,:]) > best_prob_for_last_word: #None # complete this line
            
            # Store the new best probability for the lsat word
            best_prob_for_last_word = np.max(best_probs[k,:])
    
            # Store the unique integer ID of the POS tag
            # which is also the row number in best_probs
            z[m - 1] = np.argmax(best_probs[k,:])
    ...
    pred[m - 1] = states[int(z[m - 1])]
    
    ## Step 2 ##
    # Find the best POS tags by walking backward through the best_paths
    # From the last word in the corpus to the 0th word in the corpus
    for i in range(len(corpus)-1, 0, -1): # complete this line
...
def compute_accuracy(pred, y):
    ...
    for prediction, y in zip(pred, y):
        ...
        if (word, tag) == (word, prediction): # complete this line
